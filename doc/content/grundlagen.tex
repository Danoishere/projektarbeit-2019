...
Reinforcement learning methods are categorized into value-based methods and policy-based methods \cite{tdlearning},\cite{policygradient}. Those variants differ on how they select an action $\mathcal{u}$ from a given state $\mathcal{s}$.
...
\end{gather*}
This means that for actions where the received reward $\mathcal{R}$ exceeds the expected reward $\mathcal{V(s)}$ the policy update gets multiplied by a positive advantage. Therefore, the update of the neural network gets update into a direction that favors the experienced actions based on the seen states.
...
\begin{figure}
	...
	\caption{Possible switches in the flatland environment from \cite{flatland}.}
\end{figure}
