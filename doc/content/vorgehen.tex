...
Gupta et al. find in \cite{multiagent_comp_a3c_dqn_etc}, that RL methods like Deep Q-networks and Trust Region Policy Optimization (TRPO) are not performing well in a multi-agent environment, due to the combination of experience replay and non-stationarity of the environment. Unlike the baseline version from \cite{flatlandstephan}, we do not use an experience replay buffer with older episodes. Otherwise the sampled experience might represent old agent behavior which is then learned. Instead, we directly perform updates of the neural network using the latest episodes, as proposed in \cite{a3c}.\\
